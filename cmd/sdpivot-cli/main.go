...
func runAuth(c *client, args []string) error {
	...
	if err := saveConfig(config{Server: c.server, Token: response.AccessToken}); err != nil {
		return err
	}
	fmt.Println("Authenticated with SDPivot; token saved to the CLI configuration.")
	return nil
}
...
func runQA(c *client, args []string) error {
	...
	return c.jsonRequest(http.MethodPost, "/api/v1/sdp/qa/sessions/"+url.PathEscape(*session)+"/messages", map[string]string{"content": *content}, os.Stdout)
}
