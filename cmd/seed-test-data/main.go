...
func loadConfig(path string) Config {
	...
	if cfg.Organization.AuthStatus == "" {
		cfg.Organization.AuthStatus = "trial"
	}
	if cfg.TokenUsageDays <= 0 {
		cfg.TokenUsageDays = 7
	}
	return cfg
}
...
func main() {
	...
	for _, s := range cfg.Spaces {
		...
		if errors.Is(err, gorm.ErrRecordNotFound) {
			...
			if err := db.Create(&space).Error; err != nil {
				log.Fatalf("create space %s: %v", s.Name, err)
			}
			fmt.Printf("CREATED space: %s\n", s.Name)
		} else if err != nil {
			log.Fatal(err)
		} else {
			fmt.Printf("EXISTS space: %s\n", s.Name)
		}
		...
		for _, m := range s.Members {
			var u User
			if err := db.Where("email = ?", m.Email).First(&u).Error; err != nil {
				log.Printf("warning: member email not found, skipped: %s", m.Email)
				continue
			}
			role := m.Role
			if role == "" {
		...
		for _, d := range s.Documents {
			...
			if errors.Is(err, gorm.ErrRecordNotFound) {
				...
				if err := db.Create(&doc).Error; err != nil {
					log.Fatalf("create document %s: %v", d.Title, err)
				}
	...
	for _, q := range cfg.QASessions {
		...
		if err := db.Create(&sess).Error; err != nil {
			log.Fatalf("create qa session: %v", err)
		}
		for _, msg := range q.Messages {
			...
			if err := db.Create(&m).Error; err != nil {
				log.Fatalf("create qa message: %v", err)
			}
		}
	...
	for i := 0; i < cfg.TokenUsageDays; i++ {
		...
		if err := db.Create(&u).Error; err != nil {
			log.Printf("warning: create token usage failed: %v", err)
		}
	}
