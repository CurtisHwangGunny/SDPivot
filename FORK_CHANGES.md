# SDPivot — WeKnora Fork 侵入修改记录

> 当前产品显示名为 **SDPivot（SDP）**。文中的 `smartknora_*` 表名和 `000001`–`000010` migration 文件名是 legacy 数据契约，必须保留；旧 API 前缀继续兼容。
...
| # | 文件 | 修改类型 | 行数 | 说明 |
|---|------|---------|:----:|------|
| 1 | `internal/container/container.go` | 新增 1 行 | +1 | 注册 SDPivotRouter 到 DI 容器 |
...
**新增行**:
```go
must(container.Provide(router.NewSDPivotRouter)) // smartKnora (随越·智枢) DI registration
```

**作用**: 通过 dig 依赖注入框架注册 `SDPivotRouter`，dig 自动解析其依赖：
...
type RouterParams struct {
    ...
    SDPivotRouter     *SDPivotRouter  // ← 新增
}
```
...
**作用**: 
1. 将 `SDPivotRouter` 注入到路由参数
2. 在 WeKnora 路由构建完成后，注册 SDPivot 路由

**风险评估**: 极低。`SDPivotRouter != nil` 防御性检查确保即使 DI 未注入也不崩溃。SDPivot 路由独立于 WeKnora 原有路由（`/api/v1/sdp/*`），不会冲突。
...
| 文件 | 说明 |
|------|------|
| `internal/types/smartknora.go` | **主文件** — 所有 SDPivot 类型 + DTO |
...
| 文件 | 说明 |
|------|------|
| `internal/router/smartknora.go` | SDPivotRouter 定义 + DI 构造函数 |

### 3.7 前端（待创建）
| 目录 | 说明 |
|------|------|
| `frontend/sdpivot/` | React + TDesign React 前端应用 |
...
| 变量 | 必填 | 默认值 | 说明 |
|------|:---:|--------|------|
| `SDP_JWT_SECRET` | 是 | 无（必须显式提供） | JWT 签名密钥 |

---

## 五、API 路由表

所有 SDPivot 路由挂载在 `/api/v1/sdp/`，独立于 WeKnora 原有 API。
...
SDPivot 侵入的 2 个文件在 WeKnora 上游更新时需检查：

1. **`internal/container/container.go`** — 若上游新增 handler/service，需要确认 `Provide(router.NewSDPivotRouter)` 位置仍在 `Provide(router.NewRouter)` 之前。
2. **`internal/router/router.go`** — 若上游修改 `RouterParams` 结构体或 `NewRouter` 函数体，需同步合并 `SDPivotRouter` 字段和 `RegisterRoutes` 调用。
