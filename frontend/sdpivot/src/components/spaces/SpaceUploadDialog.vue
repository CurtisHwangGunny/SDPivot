  ...
  <t-dialog :visible="visible" header="导入文档" :footer="false" width="480px" @update:visible="emit('update:visible', $event)">
    <t-tabs v-model="uploadTab">
      <t-tab-panel value="file" label="文件上传">
        <t-upload v-model="fileList" :action="`/api/v1/sdp/documents/upload`" :data="{ space_id: spaceId }" :headers="uploadHeaders" :max="20" :size-limit="{ size: 50, unit: 'MB' }" :accept="acceptFormats" multiple auto-upload @success="onUploadSuccess" @fail="onUploadFail">
...
const uploadHeaders = computed(() => ({ Authorization: `Bearer ${localStorage.getItem(STORAGE_KEYS.accessToken)}` }))
