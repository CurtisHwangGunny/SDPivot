...
func (h *SystemHandler) CreateModelConfig(c *gin.Context) {
	...
	row := modelConfigRow{
		Key:         modelConfigKeyPrefix + config.ID,
		Value:       value,
		Description: "AI model configuration: " + config.Name,
		CreatedAt:   now,
		UpdatedAt:   now,
	}
...
func (h *SystemHandler) UpdateModelConfig(c *gin.Context) {
	...
	if err := h.db.WithContext(c.Request.Context()).Model(row).Updates(map[string]any{
		"value":       value,
		"description": "AI model configuration: " + config.Name,
