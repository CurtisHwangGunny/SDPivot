...
func TestSDPivotRepresentativeRoutes(t *testing.T) {
	...
	for _, prefix := range []string{"/api/v1/sdp", "/api/v1/smartknora"} {
		...
		for _, route := range []struct {
			...
			{http.MethodGet, "/admin/update/check"},
			{http.MethodGet, "/admin/update/logs"},
			{http.MethodGet, "/admin/update/version"},
			{http.MethodGet, "/admin/logs"},
			{http.MethodGet, "/admin/logs/levels"},
			{http.MethodGet, "/admin/monitor"},
