...
def main():
    ...
    if os.environ.get("OP_DEPLOY_INTERNAL_TESTING") == "1" and test_root.startswith("/tmp/op-deploy-test."):
        ...
        if min(min_mem_mib, max_load_milli, min_disk_mib) < 0:
            fail("test resource thresholds are invalid")
    try:
        tmp = os.lstat("/tmp")
    except OSError:
    ...
    try:
        ...
        child = subprocess.Popen(
            [
                ...
                str(min_mem_mib), str(max_load_milli), str(min_disk_mib), "--", *sys.argv[1:]
            ],
