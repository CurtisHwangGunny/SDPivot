...
readonly CORE_LATEST_VERSION=80
readonly CORE_AMBIGUOUS_VERSION=12
readonly SDPIVOT_BASELINE_VERSION=12
readonly SDPIVOT_LATEST_VERSION=24
readonly CORE_MIGRATIONS_DIR=/migrations/versioned
readonly BOOTSTRAP_MIGRATIONS_DIR=/migrations/postgres-bootstrap
# The upgrade branch keeps the complete OP chain (000012..000024) in the
# postgres-bootstrap directory; the same ledger resumes it after the baseline.
readonly SDPIVOT_MIGRATIONS_DIR=/migrations/postgres-bootstrap
...
if [[ "$sdpivot_state" == "absent" ]]; then
    ...
    # The upgrade branch stores the OP baseline and all follow-up OP migrations
    # in one ordered postgres-bootstrap chain (000012 through 000024).
    ...
    sdpivot_version="$SDPIVOT_LATEST_VERSION"
else
