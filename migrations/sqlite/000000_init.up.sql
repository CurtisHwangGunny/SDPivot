...
INSERT OR IGNORE INTO tag_dimensions (id, code, name, description, sort_order) VALUES
...
CREATE TABLE IF NOT EXISTS tag_dictionary (
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (dimension_id) REFERENCES tag_dimensions(id) ON DELETE RESTRICT
);

CREATE UNIQUE INDEX IF NOT EXISTS idx_tag_dictionary_dimension_name
    ON tag_dictionary(dimension_id, name COLLATE NOCASE);
...
CREATE INDEX IF NOT EXISTS idx_document_tags_document ON document_tags(tenant_id, document_id);
